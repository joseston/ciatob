...
const TimeSlotSelector: React.FC<TimeSlotSelectorProps> = ({
  ...
  const hasAvailableSlots = Object.keys(groupedSlots).length > 0 && 
    Object.values(groupedSlots).some(slots => slots.length > 0);
    ...
    >
      {!doctorSelected ? (
        <div className="text-center py-10 text-gray-500">
          ...
          <p>Selecciona un especialista para ver la disponibilidad de horarios</p>
        </div>
      ) : loading ? (
        <div className="text-center py-10">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#46b1b9] mx-auto mb-4"></div>
          <p className="text-gray-500">Cargando disponibilidad de horarios...</p>
        </div>
      ) : hasAvailableSlots ? (
              ...
              <div key={date} className="border rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                  {slots.map((slot, index) => (
                    <motion.button
                      key={`${date}-${slot.id}-${index}`}
                      onClick={() => onSelectSlot(slot)}
                      className={`p-2 rounded-md text-sm font-medium transition-all ${
                        selectedSlot?.id === slot.id
                          ? 'bg-[#46b1b9] text-white shadow-md'
                          : 'bg-gray-100 text-gray-700 hover:bg-[#46b1b9]/10 hover:text-[#46b1b9]'
                      }`}
        ...
        // MOSTRAR WHATSAPP SOLO SI NO HAY DISPONIBILIDAD
        <div>
          <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center">
